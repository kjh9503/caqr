...
"""
python run.py --cuda --do_train --do_valid --do_test --data_path '/home/kjh9503/query2box_master/data/FB15k-237' --model 'BoxTransE' -cpu 5 --max_steps 300000 --stepsforpath 300000 --print_on_screen --geo 'box' --bn 'before' --save_checkpoint_steps 100 -b 512 -d 400 -g 24 -a 1.0 -lr 0.0001 --test_batch_size 16 --center_reg 0.2 --task 2c --offset_deepsets inductive --center_deepsets eleattention
...
from structural_table import task2len
...
from hyperrel import construct_rel_ht
...
def parse_args(args=None):
    ...
    parser.add_argument('-save', '--save_path', default=None,
                        type=str)  # main function에 codes/log directory 만들고 거기에 저장하도록 되어있음.
    ...
    parser.add_argument('--task', default='1c.2c.3c.2i.3i.ic.ci.2u.uc', type=str)
    parser.add_argument('--stepsforpath', type=int, default=0)

    ####################################################################################################################
    parser.add_argument('-sd', '--structure_dim', default=108, type=int)
    ...
    parser.add_argument('--table_normalization', action='store_true', default=False)
    ####################################################################################################################
    ####################################################################################################################
    parser.add_argument('--use_hyperrel', action='store_true', default=False)
    parser.add_argument('--rel_neigh_sample', type=int, default=0)
    parser.add_argument('--anchor_not_center_trans', action='store_true', default=False)
    ####################################################################################################################
    parser.add_argument('--offset_deepsets', default='vanilla', type=str, help='inductive or vanilla or min')
    parser.add_argument('--offset_use_center',
                        action='store_true')  # Offsetset 구하기 위한 deepset 과정에서 논문에서처럼 box embedding 전체(center, offset 모두) 사용할지 offset만 사용할지
    parser.add_argument('--center_deepsets', default='vanilla', type=str, help='vanilla or attention or mean')
    parser.add_argument('--center_use_offset',
                        action='store_true')  # Centerset 구하기 위한 attention weight 구하는 과정에서 논문에서처럼 box embedding 전체(center, offset 모두) 사용할지 center만 사용할지
...
def main(args):
    ...
    """
    args.save_path = 'logs/%s/%s/%s' % (args.data_path.split('/')[-1], args.geo, args.task+'_'+args.note)
    if args.save_path and not os.path.exists(args.save_path):
        os.makedirs(args.save_path)
    writer = SummaryWriter(args.save_path)
    # elif args.save_path and os.path.exists(args.save_path):
    #     print('====================================1======================================')
    #     cnt = 0
    #     date = str(datetime.datetime.now()).split(' ')[0][2:]
    #     while os.path.exists(args.save_path + '-' + date + '-' + str(cnt)) :
    #         cnt += 1
    #     os.makedirs(args.save_path + '-'+ date + '-' + str(cnt))
    #     args.save_path = args.save_path + '-' + date + '-' + str(cnt)
    ...
    max_qry_len = max([task2len[task] for task in tasks])
    train_ans = dict()
    valid_ans = dict()
    valid_ans_hard = dict()
    test_ans = dict()
    ...
    ####################################################################################################################
    if args.use_hyperrel :
        rel_hmap, rel_tmap, rel_h_degree, rel_t_degree, rel_h_mask, rel_t_mask = construct_rel_ht(args.data_path, args.rel_neigh_sample)
        logging.info('relation of head mapping : {}'.format(rel_hmap.shape))
        logging.info('relation of tail mapping : {}'.format(rel_tmap.shape))
    else :
        rel_hmap, rel_tmap, rel_h_degree, rel_t_degree = None, None, None, None
    ####################################################################################################################
    ...
    query2box = Query2box(
        ...
        gamma=args.gamma,
        anchor_not_center_trans=args.anchor_not_center_trans,
        rel_hmap = rel_hmap,
        rel_tmap = rel_tmap,
        rel_h_degree = rel_h_degree,
        rel_t_degree = rel_t_degree,
        max_qry_len=4,
        num_role=3,
        ...
        RE=args.RE,
        SE=args.SE,
        table_normalization=args.table_normalization,
        writer=writer,
        geo=args.geo,
        cen=args.center_reg,
    ...
    for name, param in query2box.named_parameters():
        ...
        if param.requires_grad:
            if name == 'relation_embedding' or name == 'entity_embedding':
                continue
    ...
    # query structure 별로 Data Loader 불러오기
    if args.do_train:
        ...
        if '1c' in tasks:
            ...
            train_iterator = SingledirectionalOneShotIterator(train_dataloader_tail, train_triples[0][
                -1])  # train_triples[0][-1] : qtype : '1-chain'
        ...
        if '2i' in tasks:
            ...
            train_iterator_2i = SingledirectionalOneShotIterator(train_dataloader_2i_tail, train_triples_2i[0][
                -1])  # train_triples_2i[0] : ((32, (411,)), (6463, (70,)), 0, '2-inter')
            # train_triples_2i[0][-1] : qtype : '2-inter'
    ...
    # 저장된 모델 불러오기
    if args.init_checkpoint:  # 저장된 모델 불러오기
        ...
        for step in range(init_step, args.max_steps):
            ...
            if step >= begin_pq_step and not args.train_onehop_only:
                if '2i' in tasks:
                    ...
                    # print('step{} 2i finished'.format(step))
                ...
                if '3i' in tasks:
                    ...
                    # print('step{} 3i finished'.format(step))
                    training_logs.append(log)
                ...
                if '2c' in tasks:
                    ...
                    # print('step{} 2c finished'.format(step))
                    training_logs.append(log)
                ...
                if '3c' in tasks:
                    ...
                    # print('step{} 3c finished'.format(step))
                    training_logs.append(log)
            ...
            if '1c' in tasks:
                ...
                # print('step{} 1c finished'.format(step))
                training_logs.append(log)
            ...
            if step >= warm_up_steps:
                ...
                optimizer = torch.optim.Adam(
                    filter(lambda p: p.requires_grad, query2box.parameters()),  # entity_embedding, relation_embedding
                    lr=current_learning_rate
                )
            ...
            if step % args.save_checkpoint_steps == 0:
                ...
                logging.info('Model saved at step : {}'.format(step))
            ...
            if step % args.valid_steps == 0 and args.do_valid:
