...
def parse_args(args=None):
    ...
    parser.add_argument('--rel_neigh_sample', type=int, default=120)

    parser.add_argument('--fusion_all', action='store_true', default=False)
    parser.add_argument('--var_lambda', default=0, type=float)
    ####################################################################################################################
    ...
    parser.add_argument('--note', default='orig', type=str)
...
def main(args):
    ...
    args.save_path = os.path.join(prefix, args.data_path.split('/')[-1], args.geo, args.tasks + '_' + args.note)
    ...
    if args.use_hyperrel:
        rel_hmap, rel_tmap, rel_h_degree, rel_t_degree, rel_h_mask, rel_t_mask = construct_rel_ht(args.data_path,
                                                                                                  args.rel_neigh_sample)
        logging.info('relation of head mapping : {}'.format(rel_hmap.shape))
        logging.info('relation of tail mapping : {}'.format(rel_tmap.shape))
        rel_h_degree = np.clip(rel_h_degree, a_min=0, a_max=args.rel_neigh_sample)
        rel_t_degree = np.clip(rel_t_degree, a_min=0, a_max=args.rel_neigh_sample)
    ...
    model = KGReasoning(
        ...
        table_normalization=True if args.SE else False,
        fusion_all=args.fusion_all,
        var_lambda=args.var_lambda,
        ckpt_path=args.data_path,
