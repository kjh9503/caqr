...
def construct_rel_ht(data_path, sample=0, encoder=False):
    if 'NELL' in data_path.split('/')[-1] :
        triplet_file = 'nell_train.txt'
        remapped=True
    elif 'FB15k' in data_path.split('/')[-1] :
        triplet_file = 'fb237_train.txt'
        remapped=False
    else :
    ...
    print('Triplet file : ', triplet_file)
    with open('%s/%s' % (data_path, triplet_file), 'r') as f :
        train = f.readlines()
...
def construct_rel_ht_complement(data_path, sample=0):
    if 'NELL' in data_path.split('/')[-1] :
        triplet_file = 'nell_train.txt'
        remapped=True
    elif 'FB15k' in data_path.split('/')[-1] :
...
if __name__ == '__main__' :
    ...
    # print(rel_hmap.size())
    # exit()
    # print(max(rel_h_degree))
    # exit()
    print(np.min(np.sum(rel_h_mask, axis=1)), np.max(np.sum(rel_h_mask, axis=1)))
